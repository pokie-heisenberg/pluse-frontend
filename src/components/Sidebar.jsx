...
import { Home, User as UserIcon, Bell, Bookmark, Settings as SettingsIcon, LogOut, Search as SearchIcon } from 'lucide-react';
...
export const Sidebar = () => {
  ...
  return (
    <motion.aside 
      ...
      className="fixed left-0 top-0 h-screen w-64 bg-surface-dark/40 backdrop-blur-xl border-r border-white/5 flex flex-col pt-8 pb-6 px-4 hidden md:flex z-50 shadow-2xl"
    >
      {/* Logo Area */}
      <div className="flex flex-col items-center justify-center px-4 mb-12 relative group cursor-pointer">
        {/* Glow Effect behind logo */}
        <div className="absolute inset-0 bg-gradient-to-r from-primary-500 to-accent-500 blur-2xl opacity-20 group-hover:opacity-40 transition-opacity duration-500 rounded-full scale-50"></div>
              ...
              {({ isActive }) => (
                <>
                  ...
                  <div className="relative">
                    <item.icon size={22} className={`relative z-10 transition-transform duration-300 group-hover:scale-110 ${isActive ? 'text-primary-400' : ''}`} />
      ...
      {user ? (
        ...
        >
          <div className="relative">
            <img src={user.profileImage} alt="Profile" className="w-11 h-11 rounded-full mr-3 object-cover ring-2 ring-white/10 group-hover:ring-red-500/50 transition-all" />
            <div className="absolute bottom-0 right-3 w-3 h-3 bg-green-500 rounded-full border-2 border-[#0a0a0a]"></div>
          </div>
        ...
        >
          <button onClick={() => navigate('/login')} className="w-full py-2.5 bg-primary-500 hover:bg-primary-600 text-white font-semibold rounded-xl transition-colors shadow-lg shadow-primary-500/20">
            Log In
          </button>
          <button onClick={() => navigate('/signup')} className="w-full py-2.5 bg-white/5 hover:bg-white/10 text-white font-semibold rounded-xl transition-colors border border-white/10">
            Sign Up
          </button>
        </motion.div>
      )}
